...
exports.info_delete_post = (req, res, next) => {
  ...
  async.parallel({
    ...
    info_presets: (callback) =>
      Preset.find({ infos: req.body.itemid }).exec(callback)
  }, (err, results) => {
    if (err) { return next(err); }
    if (results.info_presets.length > 0) {
      // Section has presets referencing it which must be deleted first
      res.render('item_delete', {
        title: 'Delete Info',
        item: results.info,
        dependencies: results.info_presets,
        breadcrumbs: [
          { url: '/', text: 'Home' },
          { url: '/edit', text: 'Edit content' },
          { url: '/edit/infos', text: 'Edit informative sections' },
